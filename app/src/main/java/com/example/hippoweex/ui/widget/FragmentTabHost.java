package com.example.hippoweex.ui.widget;
...
public class FragmentTabHost extends TabHost implements
    ...
    public Fragment getFragmentByTabId(String tagId) {
        for (TabInfo tabInfo : mTabs) {
            if (tagId != null && tagId.equals(tabInfo.getTag())) {
    ...
    public Fragment getCurrentFragment() {
        return mFragmentManager.findFragmentByTag(getCurrentTabTag());
    }
    ...
    //通过这个建立Fragment所以替换的FrameLayout的id
...
//					ft.detach(tab.fragment);
    ...
    public void onTabChanged(String tabId) {
        if (tabId.equals(this.mNoTabChangedTag)) {
            setCurrentTabByTag(this.mCurrentTag);
            return;
        } else {
            this.mCurrentTag = tabId;
        }
